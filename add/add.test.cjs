...
function harness() {
  ...
  vm.runInContext(fs.readFileSync(__dirname+'/add.js','utf8'),context);
...
test('answer and operand edits clear stale feedback; reveal is not marked as earned',()=>{
  ...
  h.reveal();assert.equal(h.answer.value,'22');
...
test('Random is grey and feedback is inside the equation rather than a paragraph',()=>{
  const html=fs.readFileSync(__dirname+'/add.html','utf8');
